...
namespace GraphProcessor
{
	public class ToolbarView : VisualElement
	{
		...
		protected virtual void AddButtons()
		{
			...
			AddButton("Window", OnClickWindow);
		}

		private void OnClickWindow()
		{
			var parameterType = new GenericMenu();
			foreach (var paramType in TypeCache.GetTypesDerivedFrom<PinnedElementView>())
			{
				if (paramType.BaseType == typeof(PinnedElementView))
				{
					//只允许直接继承于PinnedElementView的直接显示。
					string title = paramType.Name;
					var attr = paramType.GetCustomAttribute<WindowTitleAttribute>(true);
					if (attr != null)
					{
						title = attr.title;
					}
					parameterType.AddItem(new GUIContent(title), false, () =>
					{
						graphView.OpenPinned(paramType);
					});
				}
			}
