...
namespace GraphProcessor
{
	...
	public partial class BaseGraphView : GraphView, IDisposable
	{
		...
		public Dictionary<Type, PinnedElementView> pinnedElements { get; private set; } = new Dictionary<Type, PinnedElementView>();

		CreateNodeMenuWindow						createNodeMenu;

		protected EditorWindow m_OwnerWindow {get; private set; }

		private GraphProcessor.Custom.SearcherProvider m_SearcherProvider;
		...
		public BaseGraphView(EditorWindow window)
		{
			...
			serializeGraphElements = SerializeGraphElementsCallback;//copy操作,在节点上右键copy时触发。
			canPasteSerializedData = CanPasteSerializedDataCallback;//黏贴前的操作，右键paste时触发。
			unserializeAndPaste = UnserializeAndPasteCallback;//paste操作，右键paste时触发，下发copy时上传的数据。
            graphViewChanged = GraphViewChangedCallback;//graphview改变，删除、移动后触发
			viewTransformChanged = ViewTransformChangedCallback;//transform改变
            elementResized = ElementResizedCallback;//element的size调整

			RegisterCallback< KeyDownEvent >(KeyDownCallback);//保存、上下左右对齐快捷键。
			RegisterCallback< DragPerformEvent >(DragPerformedCallback);//拖动回调。
		...
		/// <summary>
		/// 选中节点然后ctrl+c时触发，根据传入节点，上传要复制的序列化数据。
		/// </summary>
		...
		/// <summary>
		/// graphview位置改变时触发，即按住鼠标中间拖动或滑动鼠标滚轮时触发。
		/// </summary>
		...
		void DragPerformedCallback(DragPerformEvent e)
		{
			...
			{
				//从graphview外部，级Project或Hierarchy中拖动一个节点或预设到graphview中。
		...
		void DragUpdatedCallback(DragUpdatedEvent e)
        {
            ...
            if (dragging)
                DragAndDrop.visualMode = DragAndDropVisualMode.Generic;//影响鼠标拖动在上空浮过时显示的指示图标。
		...
		{
			if (EditorWindow.focusedWindow == m_OwnerWindow)
			{
				m_SearcherProvider = ScriptableObject.CreateInstance<SearcherProvider>();
				m_SearcherProvider.Initialize(this, m_OwnerWindow);
				var searcherData = m_SearcherProvider.CreateStandardNodeMenu();
				var openPosition = context.screenMousePosition - m_OwnerWindow.position.position;
				//对齐鼠标左上角
				...
				SearcherWindow.Show(
					m_OwnerWindow,
					searcherData,
					item => m_SearcherProvider.OnSearcherWindowSelect(openPosition, item),
					openPosition,
					null,
					windowAlignment
					);
		...
		public PinnedElementView OpenPinned(Type type)
		{
			PinnedElementView view;
		...
		/// <summary>
		/// 在这个graph view中右键创建创节点时，提供能够创建的节点类型。
		/// </summary>
