...
namespace GraphProcessor
{
	...
	public partial class BaseNodeView : NodeView
	{
        ...
        protected virtual void InsertPort(PortView portView, int index)
		{
		...
		/// <summary>
		/// 将data与view对齐。以ports数据为基准，添加和移除view的port数据。
		/// </summary>
		/// <param name="ports"></param>
		/// <param name="portViews">数据调整后的portviews</param>
		/// <returns></returns>
		...
		public virtual new bool RefreshPorts()
		{
			...
			void UpdatePortViewWithPorts(NodePortContainer ports, List< PortView > portViews)
			{
				...
				{
					var p = ports.GroupBy(n => n.fieldName);//啥意思呢，就是将IEnumerable<TSource>对象转换成像Dictionary那样的kv对。返回的n.fieldName作为key。
					var pv = portViews.GroupBy(v => v.fieldName);
					p.Zip(pv, (portPerFieldName, portViewPerFieldName) => {//zip拉链，这里将回调方法Func(p_item, pv_item)=>即p和pv的每个元素相互遍历。
