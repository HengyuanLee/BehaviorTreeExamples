...
namespace GraphProcessor
{
	...
	public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
	{
        ...
        protected virtual void OnEnable()
        {
			...
			MigrateGraphIfNeeded();//数据从serializedNodes迁移到nodes。
			InitializeGraphElements();//清除无用节点和将nodes和deges放入Dictionary。
			DestroyBrokenGraphElements();//销毁两端都没有连接的edge。
			UpdateComputeOrder();//更新computeOrder。
			isEnabled = true;
			onEnabled?.Invoke();
        }
		/// <summary>
		/// 清除改名了找不到的节点和为了快速查找将nodes和edges分别放入Dictionary。
		/// </summary>
...
#pragma warning restore CS0618
		...
		public void UpdateComputeOrder(ComputeOrderType type = ComputeOrderType.DepthFirst)
		{
			...
			//找出graph输出(结束节点)并重置计算队列。
			graphOutputs.Clear();
			foreach (var node in nodes)
			{
				if (node.GetOutputNodes().Count() == 0)
					graphOutputs.Add(node);//graphOutputs：所有接受输出的节点。
				node.computeOrder = 0;
			}
		...
		int UpdateComputeOrderBreadthFirst(int depth, BaseNode node)
		{
			...
			foreach (var dep in node.GetInputNodes())
			{
				...
				//这里+=c的效果是，接着上一层depth的最后一个node的computeOrder序号。（跟着上一层depth最后一个+1）
				computeOrder += c;
			}
		...
		/// <summary>
		/// 更新计算队列，使用深度优先。就是根据深度优先设置computeOrder。
		/// </summary>
		void UpdateComputeOrderDepthFirst()
		{
			...
			foreach (var node in GraphUtils.DepthFirstSort(this))
			{
				...
				if (!node.canProcess)
					...
					node.computeOrder = computeOrder++;//叠加计算队列
			}
		}
		/// <summary>
		/// 设置右边所有node的computeOrder
		/// </summary>
		/// <param name="node"></param>
