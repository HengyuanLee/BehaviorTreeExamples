...
namespace GraphProcessor
{
	...
	public class NodePort
	{
		...
		/// <summary>
		/// 因为这个端口没有自定义推送数据的方法，所以使用这个默认的。
		/// 功能是edge把自己的左边inputField=右边outPutField的方法。
		/// 并根据TypeAdapter.cs去自动匹配不同类型的参数
		/// </summary>
...
// We keep slow checks inside the editor
		...
		public void PushData()
		{
			...
			//如果自定义IO继承了其他端口，它们也需要从passThrough获取值
		...
		public void PullData()
		{
			...
			//如果只有一个输入连接，那么可以用这里的代码处理。
			//如果有多个输入，那么需要在CustomPortsNode自定义输入方法处理
			if (edges.Count > 0)
			{
